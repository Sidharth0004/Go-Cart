...
axios.defaults.baseURL = import.meta.env.VITE_BACKEND_URL; // Ensure this is defined in .env
...
export const AppProvider = ({ children }) => {
  ...
  // ✅ Fetch user auth and cart, then update backend cart
  const fetchUser = async () => {
    try {
      const { data } = await axios.get("/api/user/is-auth");
      if (data.success) {
