...
const InputField = ({type , placeholder , name , handleChange ,address})=>(
    ...
    text-gray-500 focus:border-primary transition-all'
    type={type} 
    placeholder={placeholder} 
    name={name} 
    onChange={handleChange} 
    value={address[name]} // Ensure the input is controlled by the state
    required
     />
              ...
              <div className='flex-1 max-w-md '>  
                 <form onSubmit={onSubmitHandler} className='space-y-3  mt-6 text-sm'>
                  <div className='grid grid-cols-2 gap-4'>
                    ...
                    <button type='submit' className='w-full bg-primary text-white py-2 rounded hover:bg-primary-dull transition'>
                        Save Address    
                    </button>
                 </form>
              </div>
